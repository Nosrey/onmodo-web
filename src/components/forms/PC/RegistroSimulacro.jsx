...
import {
  editRegistroSimulacro,
  registroSimulacro,
  sendEditApplication,
} from '../../../services/FormsRequest';
...
function RegistroSimulacro() {
  ...
  const handleEdit = () => {
    ...
    editRegistroSimulacro(values, infoPrecargada._id)
      .then((resp) => {
        if (resp.error) {
          setTextAlert('Ocurrió un error');
          setTypeAlert('error');
          ...
          sendEditApplication({
            values: data,
            formId: infoPrecargada._id,
            form: '/registrosimulacro',
          }).finally((resp) => {
      ...
      {values && (
        <div>
          <div className='form'>
            ...
            {media === 'mobile' ? (
              <>
                <div className={styles.personalText} style={{ width: '100%' }}>
                  <TextField
                    style={{ width: '100%' }}
                    disabled={currentStatus === 'view'}
                    onChange={(e) => {
                      setValues({ ...values, razonSocial: e.target.value });
                    }}
                    value={values.razonSocial}
                    fullWidth
                    id='outlined-basic'
                    label='Razón Social/Contrato'
                    variant='outlined'
                  />
                </div>
                ...
                <div className={styles.personal}>
                  ...
                  <TextField
                    ...
                    disabled={currentStatus === 'view'}
                    onChange={(e) => {
                      setValues({ ...values, localidad: e.target.value });
                    }}
                      ...
                      {inputs.map((input) => (
                        <div key={input.id}>
                          <TextField
                            ...
                            value={
                              currentStatus === 'view'
                                ? infoPrecargada?.personas[index][input.prop]
                                : values.personas[index][input.prop]
                            }
              ...
              {currentStatus === 'view' || currentStatus === 'edit' ? (
                <>
                  {currentStatus === 'edit' && (
                    <div className={styles.border} {...getRootProps()}>
                      ...
                      {values.firmaDoc ? (
                        <h6>
                          {typeof values?.firmaDoc === 'string'
                            ? 'Editar Archivo'
                            : `Archivo cargado:  ${values.firmaDoc.name}`}
                        </h6>
                      ) : (
                ...
                <>
                  <p>
                    ...
                    Formularios Cargados para ser firmada por los participantes. Con todas las
                    firmas listas, desde la misma sección de Formularios Cargados, edite esta
                    planilla adjuntando en el siguiente campo el documento firmado.
                  </p>
                  <div className={styles.border} {...getRootProps()}>
                    <input {...getInputProps()} />
                    {values.firmaDoc ? (
